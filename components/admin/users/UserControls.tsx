  ...
  return (
    <div className="space-y-4">
          ...
          {(["NEW", "VERIFIED", "FLAGGED"] as const).map((tier) => {
            ...
            const configs = {
              NEW: {
                icon: Clock,
                cls: active
                  ? "bg-asb-navy text-white border-asb-navy"
            ...
            return (
              <button
                ...
                className={cn(
                  "flex flex-col items-center gap-1.5 py-3 px-2 rounded border text-xs font-bold transition-all disabled:opacity-60",
                  cls,
                )}
              >
      ...
      <ControlCard title="Trust counters">
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          >
            <p
              className={cn(
                "text-2xl font-bold",
                user.strike_count > 0 ? "text-red-700" : "text-asb-gray-500",
              )}
            >
              {user.strike_count}
            </p>
            <p
              className={cn(
                "text-xs mt-0.5",
                user.strike_count > 0 ? "text-red-600" : "text-asb-gray-500",
              )}
            >
              Strikes
            </p>
            <p className="text-[10px] text-asb-gray-400">2 = auto-FLAGGED</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="flex items-center justify-center gap-1.5 py-2 text-xs font-semibold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded transition-colors disabled:opacity-40"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 py-2 text-xs font-semibold text-asb-gray-700 bg-asb-gray-50 hover:bg-asb-gray-100 border border-asb-gray-200 rounded transition-colors disabled:opacity-40"
          >
            <RotateCcw className="w-3.5 h-3.5" />
            Reset clean posts
          </button>
      ...
      <ControlCard title="Role">
        <p className="text-xs text-asb-gray-500 mb-2">
        ...
        <select
          ...
          className="w-full h-9 text-sm px-3 rounded border border-asb-gray-200 bg-white focus:outline-none  focus:border-asb-blue cursor-pointer"
        >
      ...
      <ControlCard title="Account status">
        {user.is_active ? (
          <button
            ...
            className="w-full flex items-center gap-2 justify-center py-2.5 text-sm font-semibold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded transition-colors disabled:opacity-50"
          >
            {isPending ? (
          ...
          <button
            ...
            className="w-full flex items-center gap-2 justify-center py-2.5 text-sm font-semibold text-green-600 bg-green-50 hover:bg-green-100 border border-green-200 rounded transition-colors disabled:opacity-50"
          >
            {isPending ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
        ...
        <p className="text-[11px] text-asb-gray-400 mt-2 text-center">
          {user.is_active
      ...
      <ControlCard title="Admin notes">
        <textarea
          ...
          className="w-full text-sm px-3 py-2.5 rounded border border-asb-gray-200 bg-white resize-none focus:outline-none  focus:border-asb-blue placeholder:text-asb-gray-400"
        />
        {notesDirty && (
          <button
            disabled={isPending}
