...
// severity, pause / resume / cancel. Polls tickRun (which re-kicks a stalled
// server chain), so leaving the page never stops the run.
import * as React from "react";
import { controlRun, tickRun } from "@/app/(admin)/admin/data-quality/actions";
import { RUN_BADGE, RUN_MODE_LABEL, scopeLabel, type DqRun, type DqRunBatch } from "@/lib/dq/types";
import { Badge, Loading, fmtInt, fmtMoney } from "./ui";
import { useConsole } from "./DataQualityConsole";

export function RunProgress() {
  const { boot, nav, params, tableLabel, toast, confirm, refreshBoot } = useConsole();
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      ...
      setRun(r.data.run); setBatches(r.data.batches); setNow(Date.now());
  ...
  const pct = run.total_batches ? Math.round((run.batches_done / run.total_batches) * 100) : 0;
  ...
  const stalled = running && !!run.last_batch_at && !!now && now - new Date(run.last_batch_at).getTime() > 60_000;
  ...
  const ctl = async (a: "pause" | "resume" | "cancel") => {
    ...
    if (a === "resume") { const t = await tickRun(run.id); if (t.success) { setRun(t.data.run); setBatches(t.data.batches); } }
  };
    ...
    <section style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div className="adm-card">
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
          ...
          <Badge cls={RUN_BADGE[run.status]}>{run.status}</Badge>
          ...
          <span style={{ flex: 1 }} />
          {running && <button type="button" className="adm-btn small" onClick={() => ctl("pause")} title="Finish the current batch, then hold the cursor">Pause</button>}
          {(paused || run.status === "queued") && <button type="button" className="adm-btn small primary" onClick={() => ctl("resume")} title="Continue from the saved cursor">Resume</button>}
          {(running || paused || run.status === "queued") && <button type="button" className="adm-btn small" style={{ color: "var(--asb-red)" }} title="Stop the run; finished batches keep their issues" onClick={() => confirm({ title: "Cancel this run?", label: "Cancel run", danger: true, body: "Batches already processed keep their issues in dq_issues. The run is marked cancelled and can be resumed from its cursor later.", undo: "Runs → row → Resume from cursor.", run: () => ctl("cancel") })}>Cancel…</button>}
          ...
          {(run.status === "cancelled" || run.status === "failed") && <button type="button" className="adm-btn small" onClick={() => ctl("resume")} title="Continue from the saved primary-key cursor">Resume from cursor</button>}
        </div>
      ...
      {stalled && <div className="adm-page__warn">The server chain has not reported a batch for a minute — the next poll re-kicks it automatically.</div>}
