...
import {
  ...
  Trash2, Database, Mail, Info, Lock, X, Pencil, Anchor,
} from "lucide-react";
import {
  commitSheet, commitAll, commitSelection, undoBatch, discardBatch, listStaged, getBatch,
  findMatches, sendMatchTeaser, syncVesselPositions,
      ...
      <div style={{ display: "flex", minHeight: 340, border: `1px solid ${C.line}`, borderRadius: 10, overflow: "hidden", background: C.card }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "12px 18px", borderBottom: `1px solid ${C.line}`, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
            ...
            {/* Vessels: post the workbook's open positions into vessel_availability. */}
            {activeSheet === "vessels" && (
              <button onClick={postPositions} disabled={posting || !!busy}
                title="Create/refresh open-position postings (vessel_availability) from the workbook's STATUS=Open rows"
                style={{ ...btn("ghost"), opacity: posting || busy ? 0.6 : 1 }}>
                {posting ? <Loader2 size={15} style={{ animation: "spin 1s linear infinite" }} /> : <Anchor size={15} />} Post open positions
              </button>
            )}
