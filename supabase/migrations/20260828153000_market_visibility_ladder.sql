-- get_market_visibility also returns the archive "ladder" — the distinct
-- configured day-windows (fresh + each tier cap) — so the UI's Posted filter
-- builds its options from configuration instead of hardcoding 30/60.
