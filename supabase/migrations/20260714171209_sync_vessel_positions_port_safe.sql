-- Port-safe revision of sync_vessel_positions (supersedes 20260714133126).
-- Resolve open_port_locode against public.ports leniently (ignore case/spaces)
-- and store the REAL ports.locode so the FK vessel_availability_open_port_locode_fkey
-- never fails (the ports table has ~25 space-formatted locodes like "UA ILK"
-- while the app normalizes to no-space). If the port can't be resolved, post with
-- a NULL port (still counts as an open vessel) rather than erroring.
...
begin
  ...
  loop
    ...
    if v_status = 'OPEN' and nullif(pos->>'open_port_locode', '') is not null then
      ...
      from public.ports p
      where replace(upper(p.locode), ' ', '') = replace(upper(pos->>'open_port_locode'), ' ', '')
      limit 1;
      ...
      if v_aid is not null then
        ...
        insert into public.vessel_availability (
          ...
          v_vid, v_locode, nullif(pos->>'open_zone','')::public.zone_enum,
