-- Market Insights (issue E): switch the weekly aggregator from "posted that
-- week" (created_at within the window) to "ACTIVE/relevant that week", so the
-- editions reflect real shipping activity instead of when data happened to be
-- bulk-loaded. Mirrors the home page's "available this week" logic, parameterized
-- by the edition's own week window [p_from, p_to]:
--   • Cargo — dated: laycan OVERLAPS the week; spot/no-laycan: posted within the
--     14 days ending at week-end (recent + still circulating).
--   • Open positions — dated: open_date from ~2 weeks before the week through
--     week-end; undated: posted within the 14 days ending at week-end.
-- Everything else (scope, sub-66K, ≥5 "Other" floor, banding, grouping) is
...
live_cargo AS (
  ...
  WHERE cl.review_status = 'APPROVED'
    ...
    AND (
      ...
      -- spot / no-laycan cargo: recently posted (≤14d before week-end), still circulating
      OR (cl.laycan_from IS NULL
         AND cl.created_at <  (p_to + 1)::timestamptz
         AND cl.created_at >= (p_to - 13)::timestamptz)
    )
),
open_tonnage AS (
  ...
  WHERE va.review_status = 'APPROVED'
    ...
    AND (
      -- dated open position: opens from ~2 weeks before the week through week-end
      (va.open_date IS NOT NULL
         AND va.open_date <= p_to
         AND va.open_date >= (p_from - 14))
      -- undated open position: recently posted (≤14d before week-end)
      OR (va.open_date IS NULL
         AND va.created_at <  (p_to + 1)::timestamptz
         AND va.created_at >= (p_to - 13)::timestamptz)
    )
),
