-- Fix create_cargo_listing_v2: the INSERT listed packaging_type and
-- css_category as target columns but omitted their VALUES expressions
-- (SQL 42601 'INSERT has more target columns than expressions'), so every
-- ledger cargo post failed. Recreated with the two values supplied; the CSS
-- snapshot now lands directly in the INSERT (post-insert UPDATE removed).
