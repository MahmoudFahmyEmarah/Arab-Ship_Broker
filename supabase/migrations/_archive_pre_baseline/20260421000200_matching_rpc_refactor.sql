-- Return-table shape changed from earlier migrations, so drop first.
...
BEGIN
  ...
  WHERE
    ...
    AND (
      CASE
        WHEN va.accepts_part_cargo THEN
          v.dwt_grain >= v_cl.qty_min_mt
          AND v.dwt_grain <= v_cl.qty_max_mt * 1.20
          AND v.dwt_grain >= v_cl.qty_max_mt * 0.80
        ELSE
          v.dwt_grain >= v_cl.qty_min_mt
          AND v.dwt_grain <= v_cl.qty_max_mt * 1.10
          AND v.dwt_grain >= v_cl.qty_max_mt * 0.90
      END
    )
    AND (
    ...
    AND (
      ...
      OR COALESCE(v.grain_certified, FALSE) = TRUE
    )
    AND (
      v_cl.max_vessel_age_yr IS NULL
      OR v.build_year IS NULL
      OR (v_current_year - v.build_year::integer) <= v_cl.max_vessel_age_yr
    )
    AND (
    ...
    AND (
      v_cl.max_loa_m IS NULL
      OR v.max_loa_m IS NULL
      OR v.max_loa_m <= v_cl.max_loa_m
    )
  ORDER BY
...
BEGIN
  ...
  WHERE
    ...
    AND (
      CASE
        WHEN v_va.accepts_part_cargo THEN
          v_vessel.dwt_grain >= cl.qty_min_mt
          AND v_vessel.dwt_grain <= cl.qty_max_mt * 1.20
          AND v_vessel.dwt_grain >= cl.qty_max_mt * 0.80
        ELSE
          v_vessel.dwt_grain >= cl.qty_min_mt
          AND v_vessel.dwt_grain <= cl.qty_max_mt * 1.10
          AND v_vessel.dwt_grain >= cl.qty_max_mt * 0.90
      END
    )
    AND (
    ...
    AND (
      ...
      OR COALESCE(v_vessel.grain_certified, FALSE) = TRUE
    )
    AND (
    ...
    AND (
      cl.max_loa_m IS NULL
      OR v_vessel.max_loa_m IS NULL
      OR v_vessel.max_loa_m <= cl.max_loa_m
    )
  ORDER BY
    is_rate_aligned DESC,
    dwt_delta ASC;
