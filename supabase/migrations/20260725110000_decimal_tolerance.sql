-- Decimal cargo tolerance (user request 25 Jul): brokers quote fractional
-- MOLOO/MOLCHOPT percentages (e.g. 7.5%). tolerance_pct was smallint, which
-- silently rounded them. Widen to numeric(4,1) and stop the smallint cast in
-- create_cargo_listing_v2 (function otherwise identical to 20260724130000).
-- The v1 RPC's ::smallint payload cast still writes cleanly into numeric.

ALTER TABLE public.cargo_listings
  ALTER COLUMN tolerance_pct TYPE numeric(4,1);
