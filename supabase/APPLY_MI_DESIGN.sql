-- ════════════════════════════════════════════════════════════════════
-- Market Insights design update — lane typical-band chips (2026-06-12)
-- Run ONCE in the Supabase SQL editor.
--
-- 1) Generator v2: each top lane now carries its typical (modal) size band.
-- 2) Rebuilds the three bootstrap editions so the chips appear in W21–W23
--    (one-time delete + republish; the Monday cron owns weeks after that).
-- ════════════════════════════════════════════════════════════════════
...
-- ── Rebuild bootstrap editions with the band-carrying payload ──
...
-- Sanity: lanes should now include "band"
SELECT week_id, payload->'top_lanes'->0 AS first_lane
FROM public.market_insights_editions
WHERE week_id IN ('2026-W21','2026-W22','2026-W23') ORDER BY week_id DESC;
