...
| 3 | Stowage factor standard **m³/t** | ✅ done | Input + commodity default in m³/t; Review now shows `X.XX m³/t · NN ft³/t` (× 35.87). |
...
| 6 | Org model visible | 🟡 in progress | **Backend migration …000800 written** (organizations + organization_members + fn_my_org_ids + owner_org_id + firewall extended to org membership, append-only/backward-compatible). UI: **Posting-as chip** on Post Cargo + Review **"Posted by" + company-desk visibility** card live (org is a demo stub — `lib/portal/org.ts`). TODO: cargo/vessel **detail-panel** Posted-by/Ownership; seed orgs/members + populate owner_org_id; re-point masked views to org desk + handled_by. | The `organizations` / `organization_members` / `owner_org_id` schema + firewall-via-`fn_my_org_ids()` is **designed** (see earlier migration sketch) but **not built/seeded** — no membership/desk data in prod yet. The prototype’s `ASB_cargoOrg`/`ASB_vesselOrg` are demo stubs. Build order: (a) org migrations + desk contact, (b) seed orgs/members from a people list, (c) `owner_org_id` join, (d) Posting-as header + Review “Posted by” + detail-panel “Posted by”/Ownership reading the real org. |
